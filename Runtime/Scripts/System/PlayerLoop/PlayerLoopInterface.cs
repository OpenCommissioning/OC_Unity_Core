...
public static class PlayerLoopInterface 
{
    ...
    private static List<PlayerLoopSystem> _insertedSystems = new List<PlayerLoopSystem>();
    ...
    private static void Initialize() 
    {
        ...
        {
            foreach (var playerLoopSystem in _insertedSystems) TryRemoveSystem(playerLoopSystem.type);
    ...
    {
        if (toInsert.type == null) throw new ArgumentException("The inserted player loop system must have a marker type!", nameof(toInsert.type));
        if (toInsert.updateDelegate == null) throw new ArgumentException("The inserted player loop system must have an update delegate!", nameof(toInsert.updateDelegate));
    ...
    public static void InsertSystemBefore(PlayerLoopSystem toInsert, Type insertBefore) 
    {
        if (toInsert.type == null) throw new ArgumentException("The inserted player loop system must have a marker type!", nameof(toInsert.type));
        if (toInsert.updateDelegate == null) throw new ArgumentException("The inserted player loop system must have an update delegate!", nameof(toInsert.updateDelegate));
        if (insertBefore == null) throw new ArgumentNullException(nameof(insertBefore));
    ...
    {
        if (type == null) throw new ArgumentNullException(nameof(type), "Trying to remove a null type!");
    ...
    private static bool TryRemoveTypeFrom(ref PlayerLoopSystem currentSystem, Type type) {
        ...
        for (var i = 0; i < subSystems.Length; i++) 
        {
            ...
            if (TryRemoveTypeFrom(ref subSystems[i], type)) return true;
        }
    ...
    private static void InsertSystem(ref PlayerLoopSystem currentLoopRecursive, PlayerLoopSystem toInsert, Type insertTarget, InsertType insertType, out bool couldInsert) 
    {
        ...
        if (indexOfTarget != -1) 
        {
            ...
            {
                if (i < insertIndex) newSubSystems[i] = currentSubSystems[i];
